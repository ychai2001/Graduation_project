...
def cg_gpu(A_h, b_h, x0_h, N, max_iter=5000, tol=1e-12):
    ...
    # GPU 임시/결과 벡터 할당
    r_d = cuda.device_array(N, dtype=np.float64)
    p_d = cuda.device_array(N, dtype=np.float64)
    Ap_d = cuda.device_array(N, dtype=np.float64)
    tmp_d = cuda.device_array(N, dtype=np.float64) # 내적 계산용
    ...
    for j in range(max_iter):
        ...
        p_d_scaled = cuda.device_array(N, dtype=np.float64)
        ...
        Ap_d_scaled = cuda.device_array(N, dtype=np.float64)
        gpu.svmul[blockspergrid, threadsperblock](Ap_d, alpha, Ap_d_scaled)
        r_new_d = cuda.device_array(N, dtype=np.float64)
        ...
        p_new_d = cuda.device_array(N, dtype=np.float64)
