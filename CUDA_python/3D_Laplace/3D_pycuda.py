...
# PyCUDA 커널 정의
module = SourceModule("""
__global__ void bc_gpu(int n, double* t) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    int N = n + 2;

    if (i >= 1 && i <= n && j >=1 && j <= n) {
        // ti[i, n+1, j] = 100   # Right
        t[i * N * N + (n+1) * N + j] = 100.0;

        // ti[i, 0, j] = 100     # Left
        t[i * N * N + 0 * N + j] = 100.0;

        // ti[i, j, n+1] = 100   # Bottom
        t[i *N*N + j * N + (n+1)] = 100.0;
        ...
        // ti[n+1, i, j] = 100   # back
        t[(n+1)*N*N + i * N + j] = 100.0;
    }
}


__global__ void jacobi_gpu(int n, double* ti, double* dt) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void add_array_gpu(int n, double* ti, double* dt) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
...
def Laplace_pcd(n, tol):
    ...
    ti = np.zeros((n+2, n+2,n+2), dtype=np.float64)
    dt = np.zeros_like(ti)
